...
WHERE {
  { SELECT ?sp (max(?yearOrd) as ?maxYearOrd) WHERE {
      ...
      FILTER EXISTS { 
             ...
             FILTER NOT EXISTS { ?obs bwq:recordStatus bwq:withdrawal }
             FILTER NOT EXISTS { ?obs dct:isReplacedBy [] }
           }
    } GROUP BY ?sp 
  }

  #Rebind ?year and ?slice to continue
  ?year   a                    interval:CalendarYear ;
            interval:ordinalYear ?maxYearOrd .
  ...
  ?slice  a   bwq:BySamplingPointYearSlice ;
   ...
   # Make sure it hasn't been withdrawn or replaced
   FILTER NOT EXISTS {  ?o  dct:isReplacedBy [] }
   FILTER NOT EXISTS {  ?o  bwq:recordStatus    bwq:withdrawal; }

   # Make sure there are no later unreplaced/unwithdrawn observations 
   FILTER NOT EXISTS {
...
WHERE {
   ...
   # Pick most recent suspension record for a given ?sp ?startOfSuspension
   { SELECT ?sp ?startOfSuspension (max(?recordDateTime) as ?maxRecordDateTime) WHERE {
       ?suspension def-som:startOfSuspension        ?startOfSuspension;
                   def-sp:samplingPoint             ?sp; 
                   def-som:recordDateTime           ?recordDateTime;
                   def-bwq:sampleYear               ?sampleYear;
                   .
     } GROUP BY ?sp ?startOfSuspension
   }

 
   # Bind the corresponsing suspension record
   ?suspension def-som:startOfSuspension  ?startOfSuspension;
         def-sp:samplingPoint             ?sp; 
         def-som:recordDateTime           ?maxRecordDateTime;
         def-bwq:sampleYear               ?sampleYear;
         .
